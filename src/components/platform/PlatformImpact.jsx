...
export default function PlatformImpact() {
  ...
  return (
    <section id="impact" ref={containerRef} className="py-32 bg-primary text-white overflow-hidden relative">
      {/* Dynamic atmospheric radial glow */}
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[130%] h-[130%] bg-[radial-gradient(circle_at_center,rgba(131,171,142,0.06)_0%,transparent_60%)] pointer-events-none" />

      <div className="relative z-10 max-w-7xl mx-auto px-6 lg:px-8 w-full">
        
        {/* Header */}
        <div className="impact-reveal text-center mb-24 max-w-3xl mx-auto">
          <span className="text-[10px] font-body font-semibold uppercase tracking-[0.2em] text-verdaez-400 mb-4 inline-block px-4 py-1.5 border border-verdaez-400/30 rounded-full bg-verdaez-400/10 shadow-[0_0_15px_rgba(235,243,237,0.05)]">
            Impact Dashboard
          </span>
          ...
          <p className="font-body text-base md:text-lg text-white/70 leading-relaxed">
            Every molecule of carbon we process is accounted for. The Verdaez platform balances commercial unit economics with deep, durable environmental and social benefits.
          </p>
        </div>
              ...
              return (
                <button
                  ...
                  className={`text-left p-8 rounded-[2rem] border transition-all duration-500 flex flex-col justify-between items-start h-[220px] ${
                    isActive
                      ? "bg-white/10 border-verdaez-400/40 shadow-[0_0_30px_rgba(235,243,237,0.08)]"
                      : "bg-white/5 border-white/5 hover:border-white/20 hover:bg-white/10"
                  }`}
                >
                  <div className="w-full flex justify-between items-center">
                    <span className="text-[9px] font-body font-semibold uppercase tracking-widest text-white/50">{metric.title}</span>
                    <div className={`w-2 h-2 rounded-full ${isActive ? "bg-verdaez-400 animate-pulse" : "bg-white/20"}`} />
                  </div>
          ...
          <div className="lg:col-span-5 glass-panel bg-white/5 border border-white/10 rounded-[3rem] p-10 flex flex-col justify-between relative overflow-hidden">
            ...
            <div className="space-y-6">
              ...
              <div className="border-t border-white/10 pt-6 grid grid-cols-2 gap-4">
                <div>
                  <span className="block text-[8px] uppercase tracking-widest text-white/50">{currentMetric.statName}</span>
                  <span className="font-heading text-lg text-verdaez-200 mt-1 block">{currentMetric.statVal}</span>
                </div>
                <div>
                  <span className="block text-[8px] uppercase tracking-widest text-white/50">COMPLIANCE READINESS</span>
                  <span className="font-heading text-lg text-verdaez-200 mt-1 block">MRV Certified</span>
                </div>
              </div>
            </div>
            ...
            <div className="mt-8 bg-white/5 border border-white/10 rounded-2xl p-4 flex flex-col sm:flex-row items-center justify-between gap-4">
              <div>
                <span className="block text-[8px] uppercase tracking-widest text-white/50">UN SDG ALIGNMENT</span>
                <div className="flex flex-wrap gap-2 mt-1">
                  {currentMetric.sdg.map(tag => (
                    <span key={tag} className="text-[10px] font-body font-semibold text-verdaez-400">
                      {tag}
                    </span>
                  ))}
                </div>
              ...
              <div className="w-12 h-12 rounded-full border border-verdaez-400/30 flex items-center justify-center shrink-0">
                <svg className="w-6 h-6 text-verdaez-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="impact-reveal mt-12 bg-white/5 border border-white/10 rounded-[2rem] p-8 flex flex-col md:flex-row items-center justify-between gap-6">
          <div className="flex gap-4 items-center">
            <div className="w-10 h-10 rounded-full bg-verdaez-400/10 flex items-center justify-center border border-verdaez-400/30 shrink-0">
              <span className="font-heading text-lg font-bold text-verdaez-400">i</span>
            </div>
            <div>
              <h4 className="font-heading text-lg text-white font-normal">Measurement, Reporting, and Verification (MRV) Readiness</h4>
              <p className="font-body text-xs text-white/70 mt-0.5">
                We are actively integrating digitized carbon ledger technologies to track biomass feedstocks from the farm gate all the way to final storage.
              </p>
            </div>
          </div>
          <a href="#contact" className="btn-secondary text-white border-white/30 hover:bg-white/10 shrink-0 text-xs py-2.5 px-6">
