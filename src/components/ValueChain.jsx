...
const tiers = [
  {
    ...
    description: 'High-demand heating fuel providing immediate baseload revenues.',
    backTitle: 'Commodity Products',
    backProducts: [
      { name: 'Fuel Pellets', detail: 'Industrial heating, power generation' },
      { name: 'Industrial Biochar', detail: 'Soil amendment & carbon sequestration' },
      { name: 'Biochar Fertilizer', detail: 'Slow-release, improves soil health' },
    ],
    backHighlight: 'Immediate cash flow with proven market demand. Lowest risk entry point into the biomass value chain.',
  },
  {
    id: 2,
    ...
    description: 'Engineered biochar optimized for heavy metal and contaminant removal, and carbon sequestration.',
    backTitle: 'Industrial Products',
    backProducts: [
      { name: 'Water Filtration Media', detail: 'Heavy metal & contaminant removal' },
      { name: 'Pyrolysis Oil Fractions', detail: 'Chemical & fuel feedstock' },
      { name: 'Asphalt Modifier', detail: 'Stronger roads, lower emissions' },
    ],
    backHighlight: 'Diversified revenue across multiple industrial sectors. Higher margins through specialized processing.',
  },
  {
    id: 3,
    ...
    description: 'Next-gen high-conductivity carbon for energy storage, electronics, and advanced applications.',
    backTitle: 'Premium Products',
    backProducts: [
      { name: 'Phenolic Resin Replacement', detail: 'Sustainable material substitution' },
      { name: 'Renewable Fuel Partnerships', detail: 'Co-processing & SAF blends' },
      { name: 'Graphene & Carbon Materials', detail: 'Advanced high-value applications' },
    ],
    backHighlight: 'Proprietary technology creates deep competitive moats. Premium margins with growing global demand.',
  },
];
...
function TierCard({ tier, index, isVisible }) {
  const delayClass = ['delay-100', 'delay-200', 'delay-300'][index] || 'delay-100';
  ...
  return (
    ...
    >
      <div className="flip-card-inner">
        ...
        {/* ═══ BACK FACE ═══ */}
        <div className="flip-card-back bg-primary flex flex-col" style={{ border: '1px solid var(--color-primary-container)' }}>
          ...
          <div className="p-6 flex flex-col flex-1 overflow-auto">
            ...
            <div className="space-y-2.5 mb-5 flex-1">
              {tier.backProducts.map((product, i) => (
                ...
                >
                  ...
                  <div>
                    <p className="text-sm font-heading text-white/90 leading-snug">{product.name}</p>
                    <p className="text-[11px] text-white/40 font-body mt-0.5">{product.detail}</p>
                  </div>
                </div>
              ))}
            </div>
            ...
            {/* Highlight */}
            <div className="p-3.5 rounded-xl" style={{ background: 'rgba(255,255,255,0.04)', borderLeft: '3px solid var(--color-verdaez-400)' }}>
    ...
    <section ref={sectionRef} className="py-28 bg-background overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-14">
          <p className={`text-xs font-body font-semibold uppercase tracking-[0.2em] text-surface-tint mb-4 ${isVisible ? 'animate-fade-in-up' : 'opacity-0'}`}>
            Ratnanjali Bioenergy — One Platform
          </p>
          <h2 className={`section-heading text-center ${isVisible ? 'animate-fade-in-up delay-100' : 'opacity-0'}`}>
            From Commodity Biomass to{' '}
            <span className="italic text-surface-tint">High‑Margin</span>{' '}
            Carbon Products
          </h2>
          <p className={`section-description mx-auto text-center mt-4 ${isVisible ? 'animate-fade-in-up delay-200' : 'opacity-0'}`}>
            Three cascading technology tiers — each funding the next — transform biomass into 
            increasingly premium products with deeper competitive moats.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 lg:gap-8">
          {tiers.map((tier, i) => (
            <TierCard key={tier.id} tier={tier} index={i} isVisible={isVisible} />
          ))}
        </div>

        {/* Progression bar */}
        <div className={`mt-12 flex items-center justify-center gap-4 ${isVisible ? 'animate-fade-in-up delay-500' : 'opacity-0'}`}>
          <span className="text-xs font-body text-on-surface-variant/50 uppercase tracking-widest">Lower Risk</span>
          <div className="flex items-center gap-0.5">
            <svg className="w-4 h-4 text-surface-tint/40" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          ...
          <span className="text-xs font-body text-primary/60 uppercase tracking-widest font-semibold">Higher Value</span>
        </div>
